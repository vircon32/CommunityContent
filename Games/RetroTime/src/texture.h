...
Texture*  newTextureHorz(int textureID, int regionID, int numFrames, int startx, int starty, int width, int height, float zoomx, float zoomy)
{
	...
	define_region_matrix(regionID, startx, starty, startx+ (int)(width / numFrames)-1, starty + height -1, startx + (int)(width / numFrames)/2, starty + result->height/2, numFrames, 1, 0);
	return result;
}
...
Texture*  newTextureVert(int textureID, int regionID, int numFrames, int startx, int starty, int width, int height, float zoomx, float zoomy)
{
	...
	select_texture(textureID);   
	define_region_matrix(regionID, startx, starty, startx + width -1, starty + (int)(height / numFrames) -1, startx + result->width/2, starty + (int)(height / numFrames)/2, 1,numFrames, 0);
	return result;
}
...
Texture*  newTextureHorzVert(int textureID, int regionID, int numFramesHorz, int numFramesVert, int startx, int starty, int width, int height, float zoomx, float zoomy)
{
	...
	select_texture(textureID);   
	define_region_matrix(regionID, startx, starty, startx + (int)(width / numFramesHorz) -1, starty + (int)(height / numFramesVert) -1, startx + (int)(width / numFramesHorz)/2, starty + (int)(height / numFramesVert)/2, numFramesHorz, numFramesVert, 0);
	return result;
}
...
void drawTextureZoomed(Texture *texture, int frame, int x, int y, float zoomx, float zoomy)
{
	select_texture(texture->textureID);
	select_region(texture->regionID + frame);
