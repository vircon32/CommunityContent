...
int textfont_print_line_from_left_zoomed( textfont* tf, int left_x, int y, int* text, float scalex, float scaley )
{
    int line_length = 0;
    ...
    while( (*text) && (*text != '\n') )
    {
        // print this character
        select_region( tf->character_zero_region_id + *text );
        draw_region_zoomed_at( left_x, y );
        
        // advance in x
        left_x += (int)(textfont_get_character_width( tf, *text ) + tf->character_separation) * scalex;
        text++;
...
void textfont_print_from_left_zoomed( textfont* tf, int left_x, int y, int* text, float scalex, float scaley )
{
    select_texture( tf->texture_id );
    ...
    {
        // print this line
        int line_length = textfont_print_line_from_left_zoomed( tf, left_x, y, text, scalex, scaley );
        
        // advance to next line
        text += line_length;
        y += ((tf->character_height + tf->line_separation) * scaley);
